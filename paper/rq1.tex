...
To determine the match between a string and a pattern, the pattern was compiled using the \emph{re.compile} module in Python. An instance of \emph{re.RegexObject} \verb!m! was created using the compiled pattern. \emph{m.search()} returns an instance of \emph{re.MatchObject} \verb!m2! with the string given as the input to this function. If \verb!m2! is not \emph{None}, then that string was a match and scored 1; otherwise it scored~0.
