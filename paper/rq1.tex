...
\begin{table}[t]
\centering
\caption{3-factor ANOVA results with matching accuracy as the dependent variable, considering representation (rep), DFA size (dfa\_size), and regex length (len) as independent variables}
...
\begin{table}[t]
\centering
\caption{3-factor ANOVA results with composition accuracy as the dependent variable, considering representation (rep), DFA size (dfa\_size), and regex length (len) as independent variables}
\begin{tabular}{@{}l@{}rrrrl@{}}
  \hline
 & Df & Sum Sq & Mean Sq & F value & Pr($>$F) \\ 
  \hline
...
\subsection{Results}
Table~\ref{table:anova} shows that DFA size significantly affects the average matching accuracy for the regexes; this result is also seen in Table~\ref{table:anova2} where DFA size is shown to affect average composition, too. As a result, and since the DFA sizes vary across the pairwise comparisons, even within a representation, we present our results for  matching and composition  using each of the 41 pairs of regexes separately, rather than in aggregate over edges explored in the equivalence classes (Table~\ref{table:testedEdgesTable}, discussed later in this section). 
