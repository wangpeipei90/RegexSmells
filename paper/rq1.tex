...
Of the original 42 pairs, we report scores for only 41 pairs. Due to a design flaw, the regexes evaluated, \verb!\..*! and \verb!\.+!. were not semantically equivalent (the former is missing an escape and should be \verb!\.\.*!), so this was omitted from the . In the end, we covered  17 edges from Figure~\ref{fig:refactoringTree}.

To compute DFA size, we run both \textit{brics}~\cite{brics} and \textit{Rex}~\cite{rex} in order to get the minimal DFAs of the 60 regexes. \textit{brics} always shows the minimal DFAs but its syntax is very restricted while \textit{Rex} accepts most regular expression features. Alought most of the DFAs generated by \textit{Rex} are minimal but they are not guaranteed to be minimal. Therefore, for the regexes that are accepted by both \textit{brics} and \textit{Rex}, we chose the size of DFA generated from \textit{brics}. For the regexes only accepted by \textit{Rex}, we examined the generated DFAs and made minor changes to make sure we finally get the minimal ones.

With the representations, the DFA size and the string length of the 60 regexes, we conduct the analysis of variance (ANOVA) to find the factors that significantly affect the participants' answer accuracy. We also conduct the correlation analysis between these factors and the accuracy. We use Spearman's Rank-Order Correlation because we have no priori knowledge about these factors. Since representations are categorical data, it is excluded from correlation analysis. 
