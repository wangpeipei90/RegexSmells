...
We found that some regex expression representations are more understandable than others. For example, using a range (e.g., \verb![0-9]!) is often more understandable to a default character class (e.g., \verb![\d]!). 
We also found that DFA size for a regex significantly affects comprehension for the regexes studied. The larger the DFA for a regex (up to size eight), the more understandable it was. 
