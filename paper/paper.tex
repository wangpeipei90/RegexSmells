...
In order to identify the code smells which impact comprehension, we conducted an empirical study on 35 pairs of behaviorally equivalent but syntactically different regexes among 180 participants and evaluated the understandability of various regex language features. We found that different regex expressions can lead to different understandabilities. For example, our study showed that in expressing one or more repetitions of a character the plus (e.g., \verb!`a+'!) operator is more understandable than the kleene star operator (e.g., \verb!`aa*'!). We further analyzed regexes in GitHub to find the community standards, or the common usages of various features. Finally, we identified smelly and non-smelly regex representations based on a combination of community standards and understandability metrics. We also formed recommendations on how to transform regexes to enhance comprehension.
