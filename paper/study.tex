...
\item[C2:] Any custom character class that does not contain any shorthand representations, specifically ranges or defaults, but expresses all characters explicitly. \todoNow{does }\verb![\d][0123ab]! \todoNow{belong to this node?}
...
\todoNow{did you ensure that D1 does not subsume S3? or is the constraint that the bounds be different for D3 not actually used?}
...
Identifying C2 requires that there are no character classes using ranges or defaults. That is, all characters in all character classes in the regex must be explicit. \todoNow{is this true? or just the presence of a single character class with all explicit characters allows membership?}
