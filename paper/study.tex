...
\subsection{Analysis}
For each of the 60 regexes in the 26 groups, we computed an average matching and composition score. Within each group, we performed pairwise comparisons of the matching and composition scores to determine which representation was preferred. For example, \todoNow{add an example}. 

Although we had 26 groups of regexes evaluated by participants, we had to drop two groups of size three due to a design flaw since the regex examples performed transformations from multiple equivalence classes. For example, regex \verb!! is in C1 and T1, and was grouped with regex \verb!! in C2, T4, so it was not clear if any differences in understandability were due to the transformation between C1 and C2, or T1 and T4. 
