...
\paragraph{DBB Group}
The double-bounded group contains all regex patterns that use some repetition defined by a lower and upper boundary.  For example the pattern \verb!l(ol){1,3}! represents an `l' followed by one to three sequential `ol' patterns.  This will match `lol', `lolol' and `lololol'.

Because \verb!l(ol){1,3}! uses the curly brace repetition with a lower and upper bound, it belongs to the D1 node.  The same functional pattern can be represented as \verb!lol(ol)?(ol)?!, because the questionable (QST) modifier is used.  One question mark is equivalent to the curly brace style with a lower bound of 0, and upper bound of 1, so \verb!X?! is equivalent to \verb!X{0,1}!.  Any regex using the QST modifier belongs to the D2 node.

Whenever a repetition has a lower and upper boundary, it is possible to explode the pattern into an OR.  So using the same example, \verb!l(ol){1,3}! would become \verb!lol|lolol|lololol!, and this pattern belongs to the D3 node.  Again note that a pattern can belong to all nodes in the DBB group, for example the pattern \verb!(a|aa)X?Y{2,4}!
\paragraph{LIT Group}
